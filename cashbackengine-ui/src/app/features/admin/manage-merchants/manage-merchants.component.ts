...
@Component({
  ...
  standalone: true,
  imports: [ReactiveFormsModule, MatCardModule, MatTableModule, MatFormFieldModule, MatInputModule, MatButtonModule, MatDialogModule],
  template: `
    <div class="container">
...
export class ManageMerchantsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.api.getMerchants(0, 100).subscribe(res => this.merchants.set(res.data.content));
  }

  onCreate(): void {
    if (this.form.invalid) return;
    this.api.createMerchant(this.form.value as any).subscribe(res => {
